...
const NetworkingView: React.FC<{ currentUser: UserProfile, institutionId: string, onMessage: (userId: string) => void }> = ({ currentUser, institutionId, onMessage }) => {
  ...
  useEffect(() => {
    setUsers(db.getAllUsers(currentUser.uid, institutionId));
  ...
  useEffect(() => {
    ...
    if (activeChatId) {
       ...
       const interval = setInterval(() => {
         // Polling from LocalStorage via db
         setMessages(db.getMessages(currentUser.uid, activeChatId));
       }, 1000); 
...
const SuperAdminDashboard: React.FC<{onExit: () => void}> = ({onExit}) => {
  ...
  const [requests, setRequests] = useState<OnboardingRequest[]>(db.getOnboardingRequests());
  ...
  const refresh = () => {
    ...
    setRequests(db.getOnboardingRequests());
  };
  ...
  const handleCreateInstitution = () => {
    ...
    db.createInstitution(newInstName, newInstCode, newInstLogo, 'Newly Onboarded Institution', newInstColor);
...
const UnifiedPortal: React.FC<{ onSelect: (inst: Institution) => void, onSuperAdminLogin: () => void }> = ({ onSelect, onSuperAdminLogin }) => {
  ...
  const handleSubmitRequest = () => {
    db.submitOnboardingRequest(instName, email, contactName);
  ...
  const handleAdminSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (db.loginSuperAdmin(adminPwd)) {
        onSuperAdminLogin();
    } else {
...
const App: React.FC = () => {
  ...
  const refreshPosts = () => {
    ...
    if (currentView === ViewType.NEWSLETTER) setPosts(db.getPosts(currentInstitution.id, 'NEWSLETTER'));
    else if (currentView === ViewType.JOB_PORTAL) setPosts(db.getPosts(currentInstitution.id, 'JOB'));
    else if (currentView === ViewType.EVENTS) setPosts(db.getPosts(currentInstitution.id, 'EVENTS'));
  };
  ...
  const handleLogin = () => {
    ...
    if (loginTab === 'ADMIN') {
        result = db.loginInstAdmin(loginInput, currentInstitution.id);
    } else if (loginTab === 'STUDENT') {
      result = db.loginStudent(loginInput, currentInstitution.id);
    } else {
      result = db.loginAlumni(loginInput, currentInstitution.id);
    }
  ...
  const handleSignup = () => {
    ...
    if (loginTab === 'STUDENT') {
      ...
      user = db.signupStudent(currentInstitution.id, regName, regEmail, regBatch);
    } else if (loginTab === 'ALUMNI') {
      if (!regRoll) return alert("Roll No Required");
      user = db.signupAlumni(currentInstitution.id, regName, regRoll, regBatch, regBio);
    } else {
                  ...
                  <>
                     {loginTab === 'ADMIN' ? (
                        <div className="bg-orange-50 p-4 rounded-xl text-xs text-orange-600 font-bold mb-2">
                           <Key size={14} className="inline mr-1"/> Default password for mock: "admin"
                        </div>
                     ) : null}
                     
                     {loginTab === 'ADMIN' ? (
                       <input type="password" value={loginInput} onChange={(e) => setLoginInput(e.target.value)} placeholder="Admin Password" className="w-full p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 font-bold text-slate-700" style={{ '--tw-ring-color': currentInstitution.themeColor } as React.CSSProperties}/>
                     ) : (
                       <input value={loginInput} onChange={(e) => setLoginInput(e.target.value)} placeholder={loginTab === 'STUDENT' ? 'Email ID' : 'Roll / Student ID'} className="w-full p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 font-bold text-slate-700" style={{ '--tw-ring-color': currentInstitution.themeColor } as React.CSSProperties}/>
                     )}
